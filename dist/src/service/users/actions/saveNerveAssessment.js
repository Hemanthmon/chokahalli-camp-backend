...
const saveNerveAssessment = async (payload) => {
    ...
    try {
        ...
        const checkupRecord = await (0, findPatientCheckup_1.findPatientCheckup)(client, patient_id, constants_1.CHECKUPS.NERVE_ASSESSMENT);
        if (!checkupRecord) {
        ...
        const existing = await client.query(`SELECT id, selected_for_operation FROM nerve_assessments WHERE patient_checkup_id = $1`, [patientCheckupId]);
        const isFirstSave = !existing.rows.length;
        const previousSelectedForOperation = existing.rows[0]?.selected_for_operation ?? null;
        ...
        `, [constants_1.CHECKUP_STATUS.COMPLETED, patientCheckupId]);
          ...
          `, [constants_1.CHECKUP_STATUS.COMPLETED, patient_id]);
        ...
        else if (previousSelectedForOperation === false &&
            ...
            const bpSugarCheckup = await (0, findPatientCheckup_1.findPatientCheckup)(client, patient_id, constants_1.CHECKUPS.BP_SUGAR_TEST);
            const operationCheckup = await (0, findPatientCheckup_1.findPatientCheckup)(client, patient_id, constants_1.CHECKUPS.OPERATION_RECOMMENDATION);
            const bothStillSkipped = bpSugarCheckup?.status === constants_1.CHECKUP_STATUS.SKIPPED &&
                operationCheckup?.status === constants_1.CHECKUP_STATUS.SKIPPED;
            if (bothStillSkipped) {
                await client.query(`
          UPDATE patient_checkups
          SET status = $1, completed_at = NULL
          WHERE id = ANY($2)
          `, [constants_1.CHECKUP_STATUS.WAITING, [bpSugarCheckup.id, operationCheckup.id]]);
                await client.query(`
          UPDATE patients
          SET status = $1, updated_at = now()
          WHERE id = $2
        ...
        if (selected_for_operation) {
            (0, notifyQueueUpdated_1.notifyQueueUpdated)(queues_1.WORKFLOW_QUEUES.NERVE_ASSESSMENT, queues_1.WORKFLOW_QUEUES.FINAL_ASSESSMENT, queues_1.WORKFLOW_QUEUES.PATIENTS, queues_1.WORKFLOW_QUEUES.DASHBOARD);
        }
        else {
            (0, notifyQueueUpdated_1.notifyQueueUpdated)(queues_1.WORKFLOW_QUEUES.NERVE_ASSESSMENT, queues_1.WORKFLOW_QUEUES.PATIENTS, queues_1.WORKFLOW_QUEUES.DASHBOARD);
        }
...
exports.default = saveNerveAssessment;
