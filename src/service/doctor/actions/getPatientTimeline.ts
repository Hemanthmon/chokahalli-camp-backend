import { CHECKUPS, CHECKUP_STATUS } from "../../../common/constants";
import { pool } from "../../../db";

const getStageDetails = async (checkupId: number, patientCheckupId: number) => {
  switch (checkupId) {
    case CHECKUPS.SPECTACLE_CORRECTION: {
    ...
    case CHECKUPS.SLIT_LAMP_CHECKUP: {
      const result = await pool.query(
        `
    ...
    case CHECKUPS.NERVE_ASSESSMENT: {
      const result = await pool.query(
        `
    ...
    case CHECKUPS.OPERATION_RECOMMENDATION: {
      const result = await pool.query(
        `
...
const getPatientTimeline = async (patientId: number) => {
  ...
  const stages = await Promise.all(
    stagesResult.rows.map(async (stage) => ({
      ...
      details:
        stage.status === CHECKUP_STATUS.SKIPPED
          ? null
